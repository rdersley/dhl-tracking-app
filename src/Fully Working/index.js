...
async function getDHL(trackingNumber) {
  try {
    const response = await fetch(`${DHL_URL}?trackingNumber=${encodeURIComponent(trackingNumber)}`,
      { method: "GET", headers: { "DHL-API-Key": DHL_API_KEY } }
    );
...
export async function run() {
  ...
  for (const issue of toProcess) {
    ...
    console.log(`📬 DHL status strings for ${key}: ${JSON.stringify(statuses)}`);
    ...
    if (delivered) {
      ...
      update[DELIVERY_DATE_FIELD] = latest?.timestamp?.split("T")[0] ?? null;
      ...
      if (!signedFor) {
        signedFor = "Unknown";   // <-- IMPORTANT FIX
      }
      ...
      if (!updateRes.ok) {
        console.log(`❌ Failed field update for ${key}: ${await updateRes.text()}`);
        continue;
      }
      ...
      const comment = [
        ...
        "",
        `• **Delivered:** ${update[DELIVERY_DATE_FIELD] ?? "Unknown date"}`,
      ...
      try {
        ...
        console.log(`⚠️ Failed to add comment for ${key}: ${String(e)}`);
      }
      ...
      try {
        const tRes = await api.asApp().requestJira(
          route`/rest/api/3/issue/${key}/transitions`,
          {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ transition: { id: RESOLVED_TRANSITION_ID } })
          }
        );

        if (!tRes.ok) {
          console.log(`❌ Failed to transition ${key}: ${await tRes.text()}`);
        }
      } catch (e) {
        console.log(`⚠️ Transition error for ${key}: ${String(e)}`);
      }
    ...
    =================================*/
    if (inTransit) {
      console.log(`🚚 ${key} is In Transit`);

      if (fields[DELIVERY_STATUS_FIELD] !== "In Transit") {
        await api.asApp().requestJira(
          route`/rest/api/3/issue/${key}`,
          {
            method: "PUT",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
              fields: { [DELIVERY_STATUS_FIELD]: { value: "In Transit" } }
            })
          }
        );
      }
    ...
    console.log(`ℹ️ ${key} not delivered or transit`);
  }
