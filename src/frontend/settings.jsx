...
const UI_BUILD = "DM-COMBINED-RC-20260904-UI2";
...
const toggleRowStyle = xcss({ padding: "space.150", borderRadius: "border.radius.200", backgroundColor: "color.background.neutral.subtle" });
...
function ToggleRow({ label, description, checked, onChange, warning }) {
  return <Box xcss={toggleRowStyle}>
    <Inline spread="space-between" alignBlock="center" space="space.200">
      ...
      <Toggle isChecked={checked} onChange={onChange} label={label} />
    </Inline>
  </Box>;
}
...
function App() {
  ...
  const tabButton = (tab) => (
    <Button key={tab.id} appearance={activeTab === tab.id ? "primary" : "subtle"} onClick={() => setActiveTab(tab.id)}>
      {tab.label}
    </Button>
  );

  const renderGeneral = () => <Stack space="space.250">
    <Inline space="space.250" shouldWrap alignBlock="start">
      <Box xcss={sectionStyle}>
        <Stack space="space.250">
          <SectionHeader title="Project configuration" description="Choose the Jira projects used by Delivery Manager." />
    ...
    <Box xcss={sectionStyle}>
      <Stack space="space.250">
        <SectionHeader title="Workflow status configuration" description="Define the key statuses that connect Service Desk and Hardware." />
        <Inline space="space.200" shouldWrap alignBlock="start">
    ...
    <Box xcss={allConnectionsHealthy ? statusOkStyle : statusWarnStyle}>
      <Stack space="space.075">
        <Heading as="h3">{allConnectionsHealthy ? "Connection status — ready" : "Connection status — needs attention"}</Heading>
  ...
  const renderHardware = () => <Stack space="space.250">
    <Box xcss={sectionStyle}>
      <Stack space="space.250">
        <SectionHeader title="Hardware ticket creation" description="Configure how Delivery Manager creates and links a Hardware ticket from an SD request." />
        <Inline space="space.200" shouldWrap alignBlock="start">
    ...
    <Box xcss={subSectionStyle}>
      <Stack space="space.100">
        <Heading as="h3">How duplicate protection works</Heading>
        <Text>Before creating a Hardware ticket, Delivery Manager checks existing issue links and its own recorded creation history. It then performs a second server-side check immediately before creation.</Text>
        <Text>If a Hardware ticket already exists, the manual action warns the user instead of silently creating another one.</Text>
      </Stack>
    </Box>
  </Stack>;
  ...
  const renderTracking = () => <Stack space="space.250">
    <Box xcss={sectionStyle}>
      <Stack space="space.250">
        <SectionHeader title="DHL delivery workflow" description="Map DHL delivery events to your Service Desk workflow statuses." />
        <Inline space="space.200" shouldWrap alignBlock="start">
    ...
    <Box xcss={sectionStyle}>
      <Stack space="space.250">
        <SectionHeader title="DHL polling" description="Control which dispatched tickets are checked and how many are processed in each polling cycle." />
        <Inline space="space.200" shouldWrap alignBlock="start">
        ...
        <Text>DHL polling runs approximately every 15 minutes. Hardware reconciliation continues every 5 minutes.</Text>
      </Stack>
    </Box>

    <Box xcss={sectionStyle}>
      <Stack space="space.200">
        <SectionHeader title="Client restrictions" description="Optionally restrict DHL tracking to selected client values." />
        <ToggleRow
          label="Restrict tracking by client"
          description="When enabled, only issues whose configured client field matches one of the allowed values will be polled."
          checked={config.clientRestrictionEnabled}
          onChange={(e) => patch("clientRestrictionEnabled", e.target.checked)}
        />
        {config.clientRestrictionEnabled ? <Inline space="space.200" shouldWrap alignBlock="start">
          <Box xcss={fieldColumnStyle}><FieldBlock label="Client field" help="The Jira field that identifies the client.">{fieldSelect("clientField")}</FieldBlock></Box>
          <Box xcss={fieldColumnStyle}><FieldBlock label="Allowed client values" help="Enter values separated by commas."><Textfield value={(config.clientValues || []).join(", ")} onChange={(e) => patch("clientValues", e.target.value.split(",").map((v) => v.trim()).filter(Boolean))} /></FieldBlock></Box>
        </Inline> : null}
      </Stack>
    </Box>
  </Stack>;
  ...
  const renderMapping = () => <Stack space="space.250">
    <Box xcss={sectionStyle}>
      <Stack space="space.250">
        <SectionHeader title="Core Jira fields" description="Select the Jira fields Delivery Manager reads and updates during dispatch and delivery tracking." />
        <Inline space="space.200" shouldWrap alignBlock="start">
    ...
    <Box xcss={sectionStyle}>
      <Stack space="space.250">
        <SectionHeader title="Additional SD → HW mappings" description="Add extra Service Desk fields that should be copied into the Hardware ticket when it is created." />
  ...
  const renderAutomation = () => <Stack space="space.250">
    <Box xcss={sectionStyle}>
      <Stack space="space.200">
        <SectionHeader title="Safety & automation" description="These switches control what Delivery Manager is allowed to do automatically." />
        <ToggleRow
          label="Add internal automated comments"
          description="Adds a private Jira comment when Delivery Manager creates, repairs or updates a ticket so agents can see what happened."
          checked={config.commentsEnabled}
          onChange={(e) => patch("commentsEnabled", e.target.checked)}
        />
        <ToggleRow
          label="Allow workflow transitions"
          description="Allows Delivery Manager to move Jira issues between the configured workflow statuses when the required conditions are met."
          checked={config.transitionsEnabled}
          onChange={(e) => patch("transitionsEnabled", e.target.checked)}
        />
        <ToggleRow
          label="Automatically create missing HW tickets"
          description="When enabled, the scheduled reconciliation can create a missing Hardware ticket for an eligible SD request."
          warning="Keep this OFF during sandbox validation. Manual Create Hardware Ticket remains available for controlled testing."
          checked={config.createEnabled}
          onChange={(e) => patch("createEnabled", e.target.checked)}
        />
        {config.createEnabled ? <MessageBanner appearance="warning">Automatic Hardware creation is enabled. Only use this after manual creation and duplicate-protection tests have passed.</MessageBanner> : <MessageBanner appearance="information">Automatic Hardware creation is currently OFF. This is the recommended setting while testing in the sandbox.</MessageBanner>}
      </Stack>
    </Box>
  </Stack>;

  const renderValidation = () => <Stack space="space.250">
    <Box xcss={sectionStyle}>
      <Stack space="space.200">
        <SectionHeader title="Configuration validation" description="Check projects, statuses, Jira fields, mappings, client restrictions and Hardware creation safety before testing." />
